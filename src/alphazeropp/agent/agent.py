...
from alphazeropp.utils import THREAD_VARS
...
class Agent():
    ...
    def play_single_game(self, max_moves: int = 10_000, random_seed: int | None = None, msg = ""):
        """
        Play a single game and collect training examples with rewards.
        
        Uses either MCTS with the neural network or an external policy to select moves.
        For each move, records the game state and move probabilities. After the game ends,
        propagates rewards backwards through steps using reward_discount, then attaches
        final rewards to each training example.

        Args:
            max_moves (int, optional): Maximum number of moves before timeout. Defaults to 10_000.
            random_seed (int | None, optional): Seed for move sampling RNG. If None, uses nondeterministic seed. Defaults to None.
            msg (str, optional): Debug message prefix for logging moves. Defaults to "".

        Returns:
            list[tuple]: List of training examples, each as (state, (move_probs, reward)).
                        Returns empty list if game times out without termination/truncation.
        """
        train_examples = []
        rewards = []
    ...
    def play_and_train(self):
        ...
        multiprocessing_stash = self.push_multiprocessing() # Do preparations before going to the stage of multiprocessing
