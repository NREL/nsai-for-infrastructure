...
class BitStringPolicyValueNet(TorchPolicyValueNet):
    ...
    def __init__(self, random_seed = None, n_sites=10, n_hidden_layers = 2, hidden_size = 128, training_params = {}, device = None):
        # Compared to the network class used by cartpole, it has a different input and output size, but otherwise is the same.
        if random_seed is not None:
            torch.manual_seed(random_seed)
            torch.use_deterministic_algorithms(True, warn_only=True)

        model = PolicyValueNetModel(input_size=n_sites, output_size=n_sites, n_hidden_layers=n_hidden_layers, hidden_size=hidden_size)
